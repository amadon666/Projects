...
namespace CryptographicApplication
{
	public static class XTEA {
		/// <summary>
		/// Рекомендуемое число раундов-32, а не 64, поскольку каждая итерация выполняет два раунда шифра Фейстеля.
		/// </summary>
		private const uint Rounds = 32;

		/// <summary>
		/// Шифрует данные с помощью предоставленного ключа. Результат-кодировка Base64.
		/// </summary>
		/// <param name="data">Данные для шифрования.</param>
		/// <param name="key">Ключ, используемый для шифрования.</param>
		/// <returns></returns>
		...
		/// <summary>
		/// Расшифровывает данные с помощью предоставленного ключа.
		/// Создает исключение, если длина массива данных не кратна 8.
		/// Создает исключение, если расшифрованная длина больше, чем фактический массив.
		/// </summary>
		/// <param name="data">Зашифрованные и закодированные Base64 данные.</param>
		/// <param name="key">Ключ, используемый для расшифровки.</param>
		/// <returns></returns>
		...
		/// <summary>
		/// Шифрует данные с помощью предоставленного ключа.
		/// </summary>
		/// <param name="data">Данные для шифрования.</param>
		/// <param name="key">Ключ, используемый для шифрования.</param>
		/// <returns></returns>
		...
		/// Создает исключение, если расшифрованная длина больше, чем фактический массив.
		/// </summary>
		/// <param name="data">Шифрованные данные.</param>
		/// <param name="key">Ключ, используемый для расшифровки.</param>
		/// <returns></returns>
		...
		private static int NextMultipleOf8(int length) {
			// XTEA - это 64-битный блокчейн, поэтому наши данные должны быть кратны 64 битам
			return (length + 7) / 8 * 8; // это даст нам следующее кратное 8
		}

		/// <summary>
		/// Преобразует ключ произвольной длины в 128-битный ключ.
		/// </summary>
		/// <param name="key"></param>
		/// <returns></returns>
		private static uint[] CreateKey(byte[] key) {
			// Возможно, было бы лучше просто вычислить хэш MD5 ключа: var hash = MD5.Create().ComputeHash(key);
			// Но мы не хотим зависеть от пространства имен криптографии, потому что это увеличило бы размер сборки для некоторых платформ Unity3d.
			var hash = new byte[16];
			for(int i = 0; i < key.Length; i++) {
				hash[i % 16] = (byte)((31 * hash[i % 16]) ^ key[i]);
			}
			for(int i = key.Length; i < hash.Length; i++) { // если ключ был слишком коротким
		...
		/// <summary>
		/// Выполняет шифрование на месте предоставленного массива данных.
		/// </summary>
		/// <param name="rounds">Количество раундов шифрования, рекомендуемое значение-32.</param>
		/// <param name="v">Массив данных, содержащий два значения.</param>
		/// <param name="key">Ключевые массив из 4 значений.</param>
		...
		/// <summary>
		/// Выполняет дешифровку предоставленного массива данных на месте.
		/// </summary>
		/// <param name="rounds">Количество раундов шифрования, рекомендуемое значение-32.</param>
		/// <param name="v">Массив данных, содержащий два значения.</param>
		/// <param name="key">Ключевые массив из 4 значений.</param>
...
//	        byte[] bytesToBeEncrypted = Encrypt(byresToBeEncrypted, passwordBytes);
...
//	        byte[] passwordBytes = Encoding.UTF8.GetBytes(password);
//	        byte[] bytesToBeDecrypted = Decrypt(bytesToBeEncrypted, passwordBytes);
...
//	
	...
	public class Crc32 {
//		public static FileStream GetStream(File file, string path) {
...
//		}
        ...
        /// <returns></returns>
        public static uint Calculate(System.IO.Stream stream)
        {
    ...
    public static class MD5Helper {
        ...
        // Проверка хэшей MD5 на равенство
	...
	public static class SHA256Helper {
		...
		public static bool VerifyHash(string source, string hash) {
			using(SHA256 sha256Hash = SHA256.Create()) {
	            string hashOfInput = GetHash(source);
	            StringComparer comparer = StringComparer.OrdinalIgnoreCase;
	...
	public static class SHA1Helper {
		public static string GetHash(string source) {
			using(SHA1 sha1Hash = SHA1.Create()) {
				byte[] hash = sha1Hash.ComputeHash(Encoding.UTF8.GetBytes(source));
	...
	public static class SHA384Helper {
		public static string GetHash(string plainText)
		{
			...
			byte[] hash = sha384.ComputeHash(sourceBytes);
			return BitConverter.ToString(hash).Replace("-", String.Empty);
		}
		
		public static bool VerifyHash(string plainText, string hash)
		{
			string hashInput = SHA384Helper.GetHash(plainText);
			StringComparer comparer = StringComparer.OrdinalIgnoreCase;
    ...
    public class UnicodeManipulator {
    	UnicodeEncoding unicode = new UnicodeEncoding();
    	
    	public byte[] GetBytes(string unicode_str) {
    		return unicode.GetBytes(unicode_str);
    	}
    	
    	public string GetString(byte[] unicode_bytes) {
    		return unicode.GetString(unicode_bytes);
    	}
    }
    
    public class AsciiManipulator {
    	ASCIIEncoding ascii = new ASCIIEncoding();
    	
    	public byte[] GetBytes(string ascii_str) {
    		return ascii.GetBytes(ascii_str);
    	}
    	
    	public string GetString(byte[] ascii_bytes) {
    		return ascii.GetString(ascii_bytes);
    	}
    }
	...
	internal class TwofishBase
	{
		...
		*
		...
		private static uint f32(uint x,ref uint[] k32,int keyLen)
		{
			...
			return	unchecked((uint)((M00(b[0]) ^ M01(b[1]) ^ M02(b[2]) ^ M03(b[3]))) ^
...
//#define		REENTRANT			  1	/* nonzero forces reentrant code (slightly slower) */
		...
		***********************************************************************/
		...
		{
			return unchecked((byte)(x));//& 0xFF);
		}
		// second byte
		protected static byte b1(uint x)
		{
			return unchecked((byte)((x >> 8)));// & (0xFF));
		}
		// third byte
		protected static byte b2(uint x)
		{
			return unchecked((byte)((x >> 16)));// & (0xFF));
		}
		// fourth byte
		protected static byte b3(uint x)
		{
			return unchecked((byte)((x >> 24)));// & (0xFF));
		}
	...
	class Program
	{
		...
		{
			// TEST SUCCESS
//			string encr = XTEA.Encrypt("Hello user", "123");
//			Console.WriteLine(encr);
//			
//			string decr = XTEA.Decrypt(encr, "123");
//			Console.WriteLine(decr);
			
			//TODO TEST FAILED
//			string encr = RC2.Encrypt("Hello user", "123");
//			Console.WriteLine(encr);
//			
//			string decr = RC2.Decrypt(encr, "123");
//			Console.WriteLine(decr);
			
			// TEST Blowfish SUCCESS
			/*
			string salt = "{C42B9D23-F6F8-4AAA-8D1E-00F3A488A507}";
        	string value = "Test";
        	string encrypted = "J5APsWH6FOs=";
        	
        	Blowfish blowfish = new Blowfish(salt);
			Console.WriteLine(blowfish.Encipher(value));
			Console.WriteLine(blowfish.Decipher(encrypted));
			*/
			
			// TEST Unicode manipulators
			/*
			UnicodeManipulator unmapl = new UnicodeManipulator();
			byte[] ubytes = unmapl.GetBytes("Pi (\u03a0) and Sigma (\u03a3).");
			
			foreach(byte bt in ubytes) {
				Console.WriteLine(bt);
			}
			
			//Console.WriteLine(unmapl.GetString(ubytes));
			MessageBox.Show(unmapl.GetString(ubytes));
			*/
			
			// TEST Ascii manipulators
//			AsciiManipulator unmapl = new AsciiManipulator();
//			byte[] ubytes = unmapl.GetBytes("Pi (\u03a0) and Sigma (\u03a3).");
//			
//			foreach(byte bt in ubytes) {
//				Console.WriteLine(bt);
...
//			s = rx.Replace(s, p => new string((char) int.Parse(p.Groups[1].Value, NumberStyles.HexNumber), 1));
//			MessageBox.Show(s);
			/*
